...
window.I18N = (() => {
  ...
  const dict = {
    zh: {
      ...
      api_desc: '填入 DeepSeek API Key 后，导师将由 DeepSeek（deepseek-chat）实时驱动，能结合战况自由对话。Key 仅保存在你的浏览器 localStorage，直接调用 DeepSeek 官方 API，不经过任何第三方服务器。不填也能玩，导师会使用内置语料回答。',
      api_saved: '✅ 已保存，导师已接入 DeepSeek！',
      api_cleared: '已清除，导师切回内置语料模式。',
      api_thinking: '导师思考中……',
      api_error: '⚠️ DeepSeek 调用失败（检查 Key 或网络），先用内置语料回答你：',
    },
    en: {
      ...
      api_desc: 'Paste a DeepSeek API Key to power the advisor with DeepSeek (deepseek-chat) for free-form, battle-aware chat. The key is stored only in your browser\'s localStorage and calls the official DeepSeek API directly — no third-party server. Without a key, the advisor uses built-in responses.',
      api_saved: '✅ Saved. Advisor is now powered by DeepSeek!',
      api_cleared: 'Cleared. Advisor switched back to built-in mode.',
      api_thinking: 'Advisor is thinking...',
      api_error: '⚠️ DeepSeek call failed (check key or network). Built-in answer instead:',
    },
  };
